    ...
    <div id="wrapper">
        ...
        <div class="content-page">
            ...
            <div class="content">
                ...
                @if(session('chantier'))
                    @if(@\App\Rapport::where('chantier_id',session('chantier'))->latest()->first()->valider OR Auth::user()->can('view_rapports') OR Auth::user()->roles->contains('name','admin') OR count(@\App\Rapport::where('chantier_id',session('chantier'))->get()) == 0 )
                    ...
                    @else
                        ...
                        <p class="text-center">Rapport de la semaine du <strong>{{ @\App\Rapport::where('chantier_id',session('chantier'))->latest()->first()->debut .' au '.@\App\Rapport::where('chantier_id',session('chantier'))->latest()->first()->fin }}</strong> en attente de validation</p>
                    @endif
                @else
                    @if (@Auth::user()->personnel)
                    <a href="{{route('personnel.dashboard')}}" class="btn btn-default">
